...
def run_tests(tags):
    ...
    config_dict = yaml.load(open('./config.yml'), Loader=yaml.FullLoader)
    ...
    # check yaml-to-mux
    str_test_dir = bytes.decode(test_dir)
    test_file_name = str_test_dir.split('/')[-1]
    params_yaml_file = str_test_dir + '.data/' + \
