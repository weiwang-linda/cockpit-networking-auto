...
class CockpitNetworkingIpv6Page(SeleniumTest):
    ...
    def configure_multipul_nics_at_once(self):
        ...
        #clean up the test environment--delete manual mode of NICs
        self._clean_environment_delete_manual_ip(self.NETWORK_INFO_NICS_ENO2, self.IPV6_CONFIG_LINK)
        time.sleep(self.SLEEP_TIME)
        self._clean_environment_delete_manual_ip(self.NETWORK_INFO_NICS_ENO2, self.IPV4_CONFIG_LINK)
        time.sleep(self.SLEEP_TIME)
    ...
    # def _get_ip_via_rhvh_shell(self,nic_name,net_type):
    #     ip_a_s = self.host.execute(self.config_dict['check_ips'], timeout=self.ENTER_SYSTEM_TIMEOUT)
    #     if nic_name not in ip_a_s[1]:
    #         self.fail("The NIC '{}' is not in RHVH shell.".format(nic_name))
    #         return ""
    #     else:
    #         test1 = re.findall(r"\d+\.?\d*", nic_name)
    #         nic_num = int(test1[0])
    #         ip_text_of_nics = ip_a_s[1].split(": eno")[nic_num]

    #         if net_type == "IPv6":
    #             for line in ip_text_of_nics.splitlines():
    #                 if (self.config_dict['nic_ip_start_string_ipv6'] in line)\
    #                     and (self.config_dict['nic_ip_end_string'] in line):
    #                     return line
    #             return ""
                
    #         if net_type == "IPv4":
    #             for line in ip_text_of_nics.splitlines():
    #                 if (self.config_dict['nic_ip_start_string_ipv4'] in line)\
    #                     and (self.config_dict['nic_ip_end_string'] in line):
    #                     return line
    #             return ""
